...
module.exports.controller = (app, io, socket_list) => {
    ...
    //             helper.Dlog(files)
    //
    //             helper.CheckParameterValid(res, reqObj, ["name", "detail", "cat_id", "brand_id", "type_id", "unit_name", "unit_value", "price", "nutrition_date"], () => {
    //                 helper.CheckParameterValid(res, files, ["image"], () => {
    ...
    //
    //                     db.query("INSERT INTO `product_detail`(`cat_id`, `brand_id`, `type_id`, `name`, `detail`, `unit_name`, `unit_value`,  `price`, `created_date`, `modify_date`) VALUES (?,?,?, ?,?,?, ?,?, NOW(), NOW() ) ", [reqObj.cat_id[0], reqObj.brand_id[0], reqObj.type_id[0], reqObj.name[0], reqObj.detail[0], reqObj.unit_name[0], reqObj.unit_value[0], reqObj.price[0]], (err, result) => {
    //                         if (err) {
    //                             helper.ThrowHtmlError(err, res);
    //                             return
    //                         }
    ...
    //     var reqObj = req.body;
    //
    //     helper.CheckParameterValid(res, reqObj, ["prod_id", "name", "detail", "cat_id", "brand_id", "type_id", "unit_name", "unit_value", "price"], () => {
    //
    //         checkAccessToken(req.headers, res, (uObj) => {
    //
    //             db.query("UPDATE `product_detail` SET `cat_id`=?,`brand_id`=?,`type_id`=?,`name`=?,`detail`=?,`unit_name`=?,`unit_value`=?,`price`=?, `modify_date`=NOW() WHERE  `prod_id`= ? AND `status` = ? ", [
    //                 reqObj.cat_id, reqObj.brand_id, reqObj.type_id, reqObj.name, reqObj.detail, reqObj.unit_name, reqObj.unit_value, reqObj.price, reqObj.prod_id, "1"
    //             ], (err, result) => {
    //
    //                 if (err) {
    //                     helper.ThrowHtmlError(err, res);
